...
Vector::Vector(const gsl_vector *inVector) : Matrix(inVector->size, 1) { 
	for(unsigned int i = 0; i < inVector->size; ++i) {
		(*this)[i] = gsl_vector_get(inVector,i);
	}
}

gsl_vector* Vector::getGSL() const {
	gsl_vector* lVector =  gsl_vector_alloc(size());
	for(unsigned int i = 0; i < size(); ++i) {
		gsl_vector_set(lVector,i,(*this)[i]);
	}
	return lVector;
}
