...
{
	//Addition with an empty matrix mean adding a 0
	if( (mRows == 0) || (mCols == 0) ) {
		outMatrix = inMatrix;
		return outMatrix;
	} else if ( (inMatrix.mRows == 0) || (inMatrix.mCols == 0) ) {
		outMatrix = *this;
		return outMatrix;
	}
	
	//Addition with an matrix of size 1 = scalar
	if( (mRows == 1) && (mCols == 1 ) ) {
		return inMatrix.add(outMatrix, (*this)(0,0));
	} 
	if( (inMatrix.mRows == 1) && (inMatrix.mCols == 1) ) {
		return this->add(outMatrix, inMatrix(0,0));
	}
...
Matrix& Matrix::concatenateColumns(Matrix& outMatrix, const Matrix& inMatrix) const
{
	...
	for(unsigned int i = 0; i < inMatrix.mRows; ++i) {
		for(unsigned int j = 0; j < inMatrix.mCols; ++j) {
			outMatrix(i,j+lColOffset) = inMatrix(i,j);
		}
	}
	return outMatrix;
}
...
{
	PACC_AssertM(mCols == inMatrix.mCols, "Matrix::concatenateRows() matrix mismatch!");
	unsigned int lRowOffset = mRows;
	if(&outMatrix != this) {
		// output matrix is not self assigning
	...
	for(unsigned int i = 0; i < inMatrix.mRows; ++i) {
		for(unsigned int j = 0; j < inMatrix.mCols; ++j) {
			outMatrix(i+lRowOffset,j) = inMatrix(i,j);
		}
	}
	return outMatrix;
}
...
{
	//Multiplication with an empty matrix = 0
	if( ( (mRows == 0) && (mCols == 0) ) || ( (inMatrix.mRows == 0) && (inMatrix.mCols == 0) ) ) {
		outMatrix.resize(1,1);
		outMatrix(0,0) = 0;
		return outMatrix;
	}
	
	//Multiplication with an matrix of size 1 = scalar
	if( (mRows == 1) && (mCols == 1 ) ) {
		return inMatrix.multiply(outMatrix, (*this)(0,0));
	} 
	if( (inMatrix.mRows == 1) && (inMatrix.mCols == 1) ) {
		return this->multiply(outMatrix, inMatrix(0,0));
	}
...
/*!
 */
void Matrix::setZero(unsigned int inRows, unsigned int inCols)
{
	setRowsCols(inRows, inCols);
	vector<double>::clear();
	vector<double>::resize(inRows*inCols,0);
}
...
{
	//Substract with an empty matrix mean substracting with 0
	if( (mRows == 0) || (mCols == 0) ) {
		outMatrix = inMatrix;
		return outMatrix;
	} else if ( (inMatrix.mRows == 0) || (inMatrix.mCols == 0) ) {
	...
	//Substract with an matrix of size 1 = scalar
	if( (mRows == 1) && (mCols == 1 ) ) {
		outMatrix.setRowsCols(inMatrix.mRows, inMatrix.mCols);
		for(unsigned int i = 0; i < size(); ++i) 
			outMatrix[i] = (*this)(0,0) - inMatrix[i];
		return outMatrix;
		
	} 
	if( (inMatrix.mRows == 1) && (inMatrix.mCols == 1) ) {
		return this->subtract(outMatrix, inMatrix(0,0));
	}
...
Matrix::Matrix(const gsl_matrix *inMatrix) { 
	resize(inMatrix->size1, inMatrix->size2);
	for(unsigned int i = 0; i < getRows(); ++i) {
		for(unsigned int j = 0; j < getCols(); ++j) {
			(*this)(i,j) = gsl_matrix_get(inMatrix,i,j);
		}
	}
}

gsl_matrix* Matrix::getGSL() const {
	gsl_matrix* lMatrix =  gsl_matrix_alloc(getRows(),getCols());
	for(unsigned int i = 0; i < getRows(); ++i) {
		for(unsigned int j = 0; j < getCols(); ++j) {
			gsl_matrix_set(lMatrix,i,j,(*this)(i,j));
		}
	}
	return lMatrix;
}
...
void Matrix::SVD(Matrix& outU, Matrix& outS, Matrix& outV) const {
	
	gsl_matrix* lMatrix = getGSL();
	::SVD lSVD(lMatrix);
	...
	outU = lU; outS = lS; outV = lV;
}

Matrix Matrix::pseudoInverse(void) const {
	Matrix lMatrix; 
	return pseudoInverse(lMatrix);
}

Matrix& Matrix::pseudoInverse(Matrix& outMatrix) const {
	
	Matrix lU,lS,lV;
	SVD(lU,lS,lV);
	
	//Inverse the diagonal of S
	for(unsigned int i = 0; i < lS.getRows(); ++i){
		for(unsigned int j = 0; j < lS.getCols(); ++j) {
			if(lS(i,j) == 0) {
				throw runtime_error("Matrix::pseudoInverse((Matrix& outMatrix) element of S is nul");
			}
			lS(i,j) = 1/lS(i,j);
		}
	}
	...
	outMatrix = lV*lS*lU.transpose();
	
	return outMatrix;
}

Vector Matrix::solve(Vector& inB) const {
