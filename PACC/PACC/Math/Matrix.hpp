    ...
    */
	class Matrix : public vector<double> {
	 public:		
		...
		Matrix(const vector<double>& inVector) : vector<double>(inVector), mRows(inVector.size()), mCols(1), mPrec(15) {}
		...
		//! Set this matrix to a zero matrix with \c inRows rows and \c inCols columns.
		void setZero(unsigned int inRows, unsigned int inCols);
	...
	public:
		...
		gsl_matrix* getGSL() const;
		
		void SVD(Matrix& outS, Matrix& outV, Matrix& outD) const;
		//! Return the pseudoInverse of this matrix.
		Matrix pseudoInverse(void) const;
		//! PseudoInverse this matrix and return result through matrix \c outMatrix.
		Matrix& pseudoInverse(Matrix& outMatrix) const;
		
		//! Solve the system Ax = b, where the A is the current matrix
		Vector solve(Vector& inB) const;
