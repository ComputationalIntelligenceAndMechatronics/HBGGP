...
/*!
 *  \file   beagle/GP/src/MutationSwapSubtreeDepthSelectiveConstrainedOp.cpp
 *  \brief  Source code of class GP::MutationSwapSubtreeDepthSelectiveConstrainedOp.
...
#include "MutationSwapSubtreeDepthSelectiveConstrainedOp.hpp"
#include <algorithm>
#include <string>
 ...
 */
GP::MutationSwapSubtreeDepthSelectiveConstrainedOp::MutationSwapSubtreeDepthSelectiveConstrainedOp(
    std::string inMutationPbName,
    std::string inDistribPbName,
    std::string inName) :
  MutationSwapSubtreeConstrainedOp(inMutationPbName, inDistribPbName, inName)
{ }
 ...
 */
bool GP::MutationSwapSubtreeDepthSelectiveConstrainedOp::mutate(Beagle::Individual& ioIndividual, Beagle::Context& ioContext)
{
  ...
  Beagle_LogDebugM(
    ...
    "mutation", "Beagle::GP::MutationSwapSubtreeDepthSelectiveConstrainedOp",
    std::string("Individual tried for constrained swap subtree mutation (before): ")+
    lGPIndiv.serialize()
  );
...
//							 );
//		  
//		  //Apply only on parameters node
//		  std::vector<const std::type_info*> lDesiredTypes(1, ArgEph);
//		  RouletteT< std::pair<unsigned int,unsigned int> > lRoulette;
//		  SelectiveConstrainedSelectionOp::buildRoulette(lRoulette, lDesiredTypes, lIndividual, lContext);
//		  
//		  if(lRoulette.size() == 0) {
//			  Beagle_LogVerboseM(
//								 ioContext.getSystem().getLogger(),
//								 "mutation", "Beagle::GP::MutationSwapSubtreeDepthSelectiveConstrainedOp",
//								 string("No mutation applied, no EphemeralDouble node found.")
//								 );
//			  
//			  lDoParameterSearch = false;
//		  }
//		  if(lRoulette.size() != 0) {
...
//	  }
    ...
    Beagle_LogDebugM(
      lContext1.getSystem().getLogger(),
      "mutation", "Beagle::GP::MutationSwapSubtreeDepthSelectiveConstrainedOp",
      std::string("Individual after constrained swap subtree mutation: ")+
      lGPIndiv.serialize()
    );
  }
  else {
  ...
  return lMatingDone;
  Beagle_StackTraceEndM("bool GP::MutationSwapSubtreeDepthSelectiveConstrainedOp::mutate(Beagle::Individual& ioIndividual, Beagle::Context& ioContext)");
}
